...
import Layout from "@/lib/db/models/Layout";
...
export async function findEventById(eventid) {
  try {
    ...
    const event = await Event.findById(eventid)
      .populate(["sessions", "cars", "track", "layout"])
      .exec();
    return event;
  } catch (e) {
    console.error(e);
...
export async function findTeamById(teamid) {
  try {
    await dbConnect();
    const team = await Team.findById(teamid).populate("drivers").exec();
    return team;
  } catch (e) {
    console.error(e);
    throw new Error(e.message);
...
export async function deleteTeam(teamid) {
  try {
    ...
    await Token.deleteMany({ teamRef: teamid });
    return team;
  } catch (e) {
    console.error(e);
    throw new Error(e.message);
...
export async function removeDriverFromTeam(teamid, userid) {
  try {
    await dbConnect();
    const team = await Team.findByIdAndUpdate(teamid, {
      $pull: { drivers: userid },
